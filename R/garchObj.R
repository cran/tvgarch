garchObj <- function (par.h, xreg, order.h, fixed.g, y, flag)
{
  ...
  phi <- y / sqrt(fixed.g)
  h <- garchxSim(n = n, intercept = par.h[1], arch = par.h[2:(order.h[2]+1)], garch = par.garch, asym = par.asym, xreg = par.xreg, innovations = phi, as.zoo = FALSE, verbose = TRUE)[,"sigma2"]
